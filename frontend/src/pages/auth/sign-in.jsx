...
export function SignIn({ setAuthenticated, setIsAdmin, setIsOutlet }) {
  ...
  const isValid = formData.phone.length === 10 && formData.password.length > 0 && agree;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'phone') {
      if (value) {
        ...
        // Set the cleaned phone number (without country code)
        setFormData((prevData) => ({
          ...prevData,
          phone: cleanedValue,
        }));

        // Validate the phone number to check if it's exactly 10 digits
        setPhoneNumError(cleanedValue.length === 10 ? '' : 'Please enter a valid phone number.');
      } else {
        setPhoneNumError('Please enter a valid phone number.');
      }
    }
  ...
  const validatePhoneNumber = (phone) => {
    // A regex for validating phone numbers (assuming India, modify as needed)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call the login function for users (no role distinction)
      const response = await loginUser(formData); 
      const token = response.token;
      const role = response.role || 'user'; // Default role to 'user' if not provided
  ...
  return (
    <section className="relative flex min-h-screen flex-col lg:flex-row">
      ...
      <div className="w-full lg:w-1/2 h-screen flex flex-col justify-center items-center px-6 lg:px-12 bg-white bg-opacity-90">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mx-auto max-w-md">
          ...
          <div className="flex items-center">
            <Checkbox
              ...
              onChange={handleAgreeChange}
            />
            <Typography variant="small" className="ml-2">{t('agreeTerms')}</Typography>
          </div>
        ...
        {showAlert && (
          <div className="mt-4">
            <Typography variant="small" color={error ? 'red' : 'green'} className="text-center">
              {error || alertMessage}
            </Typography>
          </div>
        )}
