...
const CropForm = () => {
  ...
  const [cropDetails, setCropDetails] = useState([
    {
      ...
      image: '', // Will store image URL after it's selected
      category: '', // Store the category of the crop
  ...
  const handleImageChange = (e, index) => {
    ...
    if (file) {
      ...
      const updatedCrops = [...cropDetails];
      updatedCrops[index].image = imageUrl; // Store image URL for preview
      setCropDetails(updatedCrops);
    }
  };
  ...
  const handleAddCrop = () => {
    setCropDetails([
      ...cropDetails,
      {
        ...
        projectionTimelineValue: '',
        image: '',
        category: '', // Adding category field for new crop
        createdAt: new Date().toISOString().split('T')[0], // default to today's date
      },
    ]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/users/crops/save', cropDetails, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });

      console.log('Crops saved successfully:', response.data);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-8 px-4">
      <div className="w-full max-w-4xl bg-white p-6 rounded-lg shadow-lg">
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Save Crops</h2>

        <form onSubmit={handleSubmit} className="space-y-6">
          {cropDetails.map((crop, index) => (
            <div key={index} className="bg-gray-100 p-6 rounded-lg shadow-sm border border-gray-300">
              <h3 className="text-lg font-semibold text-gray-700 mb-4">Crop {index + 1}</h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Crop Name */}
                <div>
                  <label htmlFor={`cropName-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Crop Name</label>
                  <input
                    type="text"
                    id={`cropName-${index}`}
                    name="cropName"
                ...
                {/* Actual Production */}
                <div>
                  <label htmlFor={`actualProduction-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Actual Production</label>
                  <input
                    type="number"
                    id={`actualProduction-${index}`}
                ...
                <div>
                  <label htmlFor={`projectedProduction-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Projected Production</label>
                  <input
                    type="number"
                    id={`projectedProduction-${index}`}
                ...
                {/* Cultivation Land Value */}
                <div>
                  <label htmlFor={`cultivationLandValue-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Cultivation Land Value</label>
                  <input
                    type="number"
                    id={`cultivationLandValue-${index}`}
                ...
                {/* Land Value Unit */}
                <div>
                  <label htmlFor={`landValueUnit-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Land Value Unit</label>
                  <input
                    type="text"
                    id={`landValueUnit-${index}`}
                    name="landValueUnit"
                ...
                {/* Cost */}
                <div>
                  <label htmlFor={`cost-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Cost</label>
                  <input
                    type="number"
                    id={`cost-${index}`}
                    name="cost"
                ...
                {/* Project Cost */}
                <div>
                  <label htmlFor={`projectCost-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Project Cost</label>
                  <input
                    type="number"
                ...
                {/* Projection Timeline Type */}
                <div>
                  <label htmlFor={`projectionTimelineType-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Projection Timeline Type</label>
                  <input
                    type="text"
                    id={`projectionTimelineType-${index}`}
                ...
                {/* Projection Timeline Value */}
                <div>
                  <label htmlFor={`projectionTimelineValue-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Projection Timeline Value</label>
                  <input
                    type="number"
                    id={`projectionTimelineValue-${index}`}
                ...
                {/* Category */}
                <div>
                  <label htmlFor={`category-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Category</label>
                  <select
                    id={`category-${index}`}
                    name="category"
                    value={crop.category}
                ...
                <div>
                  <label htmlFor={`image-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Image Upload</label>
                  <input
                    type="file"
                    id={`image-${index}`}
                    name="image"
                ...
                {/* Created At */}
                <div>
                  <label htmlFor={`createdAt-${index}`} className="block text-sm font-medium text-gray-600 mb-2">Created At</label>
                  <input
                    type="date"
                    id={`createdAt-${index}`}
                    name="createdAt"
          ...
          <div className="flex justify-between items-center">
            <button
              ...
              className="px-4 py-2 bg-green-500 text-white rounded-md shadow-sm hover:bg-green-600 focus:ring-2 focus:ring-green-500"
            >
              Add Another Crop
            </button>
            <div className="space-x-4">
              <button
                type="submit"
                className="px-6 py-3 bg-blue-600 text-white rounded-md shadow-sm hover:bg-blue-700 focus:ring-2 focus:ring-blue-500"
              >
                Save Crops
              </button>
            </div>
