...
const UserDetails = () => {
    ...
    const handleCreateRole = async () => {
        if (roleInput) {
            try {
                const response = await axios.post('http://localhost:8080/users/admin/roles/create', {
                    roleName: roleInput
                });
    ...
    const handleEdit = async () => {
        if (!roleInput || !dialogOldRole || !dialogUserId) {
            setError('All fields are required.');
            return;
        }

        if (roleInput === dialogOldRole) {
            setError('New role name must be different from the old role name.');
            return;
        }

        if (!isValidEmail(dialogUserId)) {
            setError('Invalid email address.');
            return;
        }

        try {
            const response = await axios.put('http://localhost:8080/users/admin/edit-role', null, {
                params: {
                    email: dialogUserId,
                            ...
                            <td style={styles.tableCell}>
                                {user.role ? (
                                    <span style={styles.roleBadge}>
                                        {/* {user.role.name} (ID: {user.role.id}) */}
