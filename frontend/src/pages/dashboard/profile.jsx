...
export function Profile({ userId }) { // Accept userId as a prop
  const { t, i18n } = useTranslation();
  const classes = useStyles();
  const [formData, setFormData] = useState({
    fullName: '',
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        setFileState({ previewUrl: userData.profilePicture || null }); // Assuming the user object contains profilePicture
      } catch (error) {
